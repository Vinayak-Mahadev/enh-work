...
from 
	...
	from 
		(select 
			...
			temp.file_id_n, 
			case when temp.metrics_key_v is not null and coalesce(met.metrics_id_n, 0) = 0 then kpi.update_analytics_failed_row('tr_temp_rguga_injection_aggr', temp.temp_id_n, 'metric_field%Metric mapping not found for metric_field : ' || temp.metrics_key_v) else 1 end as id_flag 
		from 
			...
			left  join kpi.ms_metrics_master met on temp.metrics_key_v = met.ext_reference_id_v and met.metrics_type_n in (24,28) 
