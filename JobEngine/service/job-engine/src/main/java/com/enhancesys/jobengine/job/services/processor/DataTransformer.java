...
import com.enhancesys.jobcommon.condition.ExpressionProcessor;
import com.enhancesys.jobcommon.mathematical.MathematicalProcessor;
...
public class DataTransformer extends JobProcessor
{
	@Autowired
	...
	public void init(String pipeLineName, String processorName, JSONObject processorConfig, JobConfigurationReader jobConfigurationReader, JSONObject jobParameters) throws Exception
	{
		...
		try
		{
			...
			configurationObj = (JSONObject) jobConfigData.get(DataConstants.CONFIGURATION);
			lookupObj = (JSONObject) jobConfigData.get(DataConstants.LOOKUP);
			multipleRowConf = (JSONObject) jobConfigData.get(DataConstants.MULTIPLE_ROW_CONF);
...
//			log.error("DataObject : " + jobObject);
			...
			if(lookupObj != null)
			{
				...
				for(Object dataObj : dataList)
				{
					...
					fieldLevel : for(Object fieldKey : configurationObj.keySet())
					{
						...
						if(fieldObject.get(DataConstants.FIELD) instanceof JSONArray)
							...
							field = fieldObject.get(DataConstants.FIELD);
						
						if(field != null)
						{
							if(field.toString().contains("."))
							{
								values = Util.getFieldValue(field.toString(), dataObject);
							...
							if(fieldObject.containsKey(DataConstants.F_LOOKUP))
							{
								...
								for(Object fieldLookupObj : (JSONArray) fieldObject.get(DataConstants.F_LOOKUP))
								{
									...
									if(fieldArr != null)
									{
										if(mappingArr != null)
										{
											log.info("inside Mapping arr : " + mappingArr);
										...
										{
											lookupMapKey = fieldLookupObject.get(DataConstants.NAME).toString() + "~" + field.toString() + "~" + fieldLookupObject.get(DataConstants.MAPPING_FIELD).toString() + "~" + fieldValue;
...
//								log.info("FieldValue : " + fieldValue);
					...
					for(Object exFieldObject : expressionFields)
					{
						...
						log.info("Process Expression.." + fieldObject);
			...
			log.info(pipeLineName + " : " + processorName + " : DataObject 1 : " + jobObject);
...
//				log.error("queryConf : " + queryConf);
				if(queryConf.get(DataConstants.PARAM_TYPE) != null)
				{
					...
					{
						values = Util.getFieldValue(queryConf.get(DataConstants.PARAM_VALUE).toString(), dataObject);
						if(values != null && !values.isEmpty())
...
//				log.error("lookupObject : " + lookupObject);
				if(fieldLookupObject.get(DataConstants.QUERY) != null)
					...
					queryObject = Util.prepareQuery(lookupObject, dataObject);
				if(lookupObject.get(DataConstants.PARAMETERS) != null)
					Util.prepareQueryWithParams(lookupObject, queryObject, dataObject);
...
//			log.error("lookupDBObject from DB : " + lookupDBObject);
	...
	private DBObject getLookupObject(JSONObject lookupObject, JSONObject fieldLookupObject, JSONObject dataObject, JSONArray fieldArr) throws GenericProcessorException
	{
		...
		try
		{
			...
			log.info("mappingArr : " + mappingArr + " : fieldArr : " + fieldArr);
			...
			if(fieldValue != null && !fieldValue.toString().trim().isEmpty())
			{
				if(fieldObject.containsKey(DataConstants.FETCH_ON_CONDITION))
				{
					if(fieldValue instanceof JSONObject)
					{
						...
						log.info("fetchObject : " + fetchObject);
						...
						for(Object fieldCondObj : (JSONArray) fieldObject.get(DataConstants.FETCH_ON_CONDITION))
						{
							...
							log.info("fieldCondObject : " + fieldCondObject);
							if(fieldCondObject.get(DataConstants.COND_OPERATION) != null)
							{
								...
								{
									values = Util.getFieldValue(fieldCondObject.get(DataConstants.COND_FIELD).toString(), fetchObject);
									if(values != null && !values.isEmpty())
						...
						if(fieldCondObject.get(DataConstants.COND_FIELD) != null)
						{
							if(fieldCondObject.get(DataConstants.COND_FIELD) instanceof JSONArray)
							{
								if(fieldCondObject.get(DataConstants.COND_OPERATION) != null)
								{
									...
									if(condFieldArr.size() > 1)
									{
										...
										condFields : for(int i = 0; i < condFieldArr.size(); i ++)
										{
											if(i + 1 < condFieldArr.size())
											{
												...
												{
													values = Util.getFieldValue(condFieldArr.get(i).toString(), dataObject);
													if(values != null && !values.isEmpty())
												...
												{
													values = Util.getFieldValue(condFieldArr.get(i+1).toString(), dataObject);
													if(values != null && !values.isEmpty())
									...
									{
										log.error("Please configure more than 1 condition fields for field : " + fieldObject.get(DataConstants.FIELD));
							...
							else
							{
								...
								else
								{
									if(fieldCondObject.get(DataConstants.COND_FIELD).toString().contains("."))
									{
										values = Util.getFieldValue(fieldCondObject.get(DataConstants.COND_FIELD).toString(), dataObject);
										if(values != null && !values.isEmpty())
											conditionFieldValue = values.iterator().next().toString();
									}
									else
...
//				log.info("tempValue : " + tempValue);
			...
			{
				log.info("date tempValue : " + tempValue + " Format : " + fieldObject.get(DataConstants.DATE_FORMAT).toString() + " : " + tempValue.getClass());
			...
			log.error("fieldObject : " + fieldObject + " : tempValue : " + tempValue);
			return tempValue;
		}
		catch(Exception exception)
		{
...
//					log.error("fieldLookupObject.get(DataConstants.PATH) : " + fieldLookupObject.get(DataConstants.PATH) + " : Data Object : " + dataObject);
					values = Util.getFieldValue(fieldLookupObject.get(DataConstants.PATH).toString(), dataObject);
					if(values != null && !values.isEmpty())
					{
						fieldValue = values.iterator().next(); 
...
//						log.info("count : " + count + " : " + fieldValue);
			...
			if(fieldObject.containsKey(DataConstants.OPERATION))
			{
				...
				if(fieldLookupObject.get(DataConstants.OPERATION_TYPE) != null)
				{
					if(DataConstants.MINUS.equalsIgnoreCase(fieldLookupObject.get(DataConstants.OPERATION_TYPE).toString()))
					{
						...
						{
							values = Util.getFieldValue(fields.get(0).toString(), dataObject);
							if(values != null && !values.isEmpty())
						...
						for(int i = 0; i < fields.size() - 1; i++)
						{
							...
							{
								values = Util.getFieldValue(fields.get(i+1).toString(), dataObject);
								if(values != null && !values.isEmpty())
								{
			...
			tempObject.put(fieldKey.toString(), fieldValue);
			log.info(queryObject.get("ord_master_id") + " : " + queryObject.get("prd_id") + " : " + fieldValue);
		}
		catch(Exception exception)
		{
			log.error(exception.getMessage(), exception);
	...
	{
		ExpressionProcessor exProcessor = null;
		MathematicalProcessor mathProcessor = null;
...
//				log.error("fieldCondObject : " + fieldCondObject);
				if(fieldCondObject.get(DataConstants.COND_FIELD) != null)
				{
					if(fieldCondObject.get(DataConstants.COND_OPERATION) != null)
					{
						...
						{
							values = Util.getFieldValue(condField.toString(), dataObject);
							if(values != null && !values.isEmpty())
								val_1 = values.iterator().next().toString();
						}
						else
...
//			log.error("dataObject : " + dataObject);
			...
			if(fieldObject.get(DataConstants.EXPRESSION_TYPE) != null && !fieldObject.get(DataConstants.EXPRESSION_TYPE).toString().trim().isEmpty() 
				&& DataConstants.CONDITIONAL.equalsIgnoreCase(fieldObject.get(DataConstants.EXPRESSION_TYPE).toString().trim()))
			{
				dataMap = prepareDataMap(expression, fieldObject, dataObject);
				exProcessor = new ExpressionProcessor();
				exProcessor.init(expression);
				expressionResult = exProcessor.calculateExpression(dataMap);
			}
			else
			{
				jsonData = prepareJSONData(expression, fieldObject, dataObject);
				mathProcessor = new MathematicalProcessor();
//				log.error("expression : " + expression);
...
//					log.error(field.toString() + " : " + fieldValue);
	...
	{
		log.info("Entry processMultipleRowConf .. " + multipleRowConf + " : dataObject : " + dataObject);
		...
		try
		{
			...
			for(Object multipleRowName : multipleRowConf.keySet())
			{
				...
				if(field.toString().contains("."))
				{
					...
					if(values != null && !values.isEmpty())
					{
						...
						{
							log.info("fieldVal : " + fieldVal + " : " + fieldVal.getClass());
							if(fieldVal instanceof JSONObject)
							{
								...
								for(Object fieldName : fieldConf)
								{
									...
									log.info("fieldMultiRowConf : " + fieldMultiRowConf);
								...
								log.info("flag : " + flag + " : " + tempDataObject);
								if(flag)
				...
				{
					fieldValue = dataObject.get(field.toString()) != null ? dataObject.get(field.toString()) : "";
					log.info("fieldValue : " + fieldValue + " : " + fieldValue.getClass());
			...
			log.info("newDataList : " + newDataList.size());
