...
from
   ...
   from
		(select 
			...
			coalesce(mpc.aggregation_id_n, 0) as actor_id_n,
			temp.event_type_n,
			temp.metrics_key_v,
			...
			temp.file_id_n,
			case
				when temp.actor_key_v is not null and coalesce(mpc.aggregation_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_ontime_payment_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v)
				when temp.source_key_v is not null and coalesce(clus.lookup_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_ontime_payment_aggr', temp.temp_id_n, 'source_field%Actor mapping not found for source_field : ' || temp.source_key_v)
			else 1
			end as id_flag
		from 
			kpi.tr_temp_ontime_payment_aggr temp
			left join kpi.ms_org_master mpc on temp.actor_key_v = mpc.ref_code_v
			left join kpi.ms_lookup_master clus on clus.lookup_type_n = (select lookup_type_n from kpi.ms_lookup_type_master where ext_lookup_type_n = 88) and temp.source_key_v = clus.ext_ref_code_v
	...
	left join kpi.tr_monthly_ontime_payment_aggr monthly1 on temp1.month_id_n = monthly1.month_id_n
