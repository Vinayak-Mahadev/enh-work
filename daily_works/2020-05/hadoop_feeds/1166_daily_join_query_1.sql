   ...
   from
		(select 
			...
			case 
				...
				then kpi.update_analytics_failed_row('tr_temp_primary_mobo_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v)
			else 1
			end as id_flag
		from 
			kpi.tr_temp_primary_mobo_aggr as temp
			left join kpi.ms_org_master mpc on temp.actor_key_v = mpc.ref_code_v
		where temp.file_id_n = ?
		) result
	where 
		result.id_flag = 1
	group by 
	...
	left join kpi.tr_daily_primary_mobo_aggr daily1 on temp1.id_n = daily1.day_id_n
