--1177_DAILY_JOIN_QUERY_1=
select 
	1177 as interface_id_n,
	temp1.id_n,
	daily1.day_id_n,
	temp1.actor_type_n,
	temp1.actor_id_n,
...
from
	...
	from
		(select 
			...
			coalesce(clus.lookup_id_n, 0) as actor_id_n,
			temp.event_type_n,
			temp.metrics_key_v,
			...
			coalesce(ins.metrics_id_n, 0) as instance_id_n,
			temp.file_id_n,
			case
				when temp.actor_key_v is not null and coalesce(clus.lookup_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_cross_reload_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v)
				when temp.metrics_key_v is not null and coalesce(met.metrics_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_cross_reload_aggr', temp.temp_id_n, 'metric_field%Metric mapping not found for metric_field : ' || temp.metrics_key_v)
				when temp.instance_key_v is not null and coalesce(ins.metrics_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_cross_reload_aggr', temp.temp_id_n, 'instance_field%Instance mapping not found for instance_field : ' || temp.instance_key_v)
			else 1
			end as id_flag
		from 
			kpi.tr_temp_cross_reload_aggr temp
			left join kpi.ms_lookup_master clus on clus.lookup_type_n = (select lookup_type_n from kpi.ms_lookup_type_master where ext_lookup_type_n = 88) and temp.actor_key_v = clus.ext_ref_code_v
			left join kpi.ms_metrics_master met on temp.metrics_key_v = met.ext_reference_id_v and met.metrics_type_n = 23
			left join kpi.ms_metrics_master ins on temp.instance_key_v = ins.ext_reference_id_v and ins.metrics_type_n = 22
		where 
			temp.file_id_n = ?
		) result
	where 
	...
	left join kpi.tr_daily_cross_reload_aggr daily1 on temp1.id_n = daily1.day_id_n
