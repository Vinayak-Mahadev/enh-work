--1173_MONTHLY_JOIN_QUERY_1=
select 
	1173 as interface_id_n,
   ...
   from
		(select 
			...
			case
				...
				then kpi.update_analytics_failed_row('tr_temp_mobo_usage_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v)
				when temp.source_key_v is not null and coalesce(site.site_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_mobo_usage_aggr', temp.temp_id_n, 'source_field%Source mapping not found for source_field : ' || temp.source_key_v)
			else 1
			end as id_flag
		from 
			kpi.tr_temp_mobo_usage_aggr temp
	...
	left join kpi.tr_monthly_mobo_usage_aggr monthly1 on temp1.month_id_n = monthly1.month_id_n
