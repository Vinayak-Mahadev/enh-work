...
FROM
               ...
               where
                  ...
                  where
                     ...
                     CASE
                        ...
                        THEN
                           kpi.update_analytics_failed_row('tr_temp_secondary_mobo_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v) 
                        WHEN
                           temp.source_key_v IS NOT NULL 
                           AND COALESCE(mpc.aggregation_id_n, 0) = 0 
                        THEN
                           kpi.update_analytics_failed_row('tr_temp_secondary_mobo_aggr', temp.temp_id_n, 'source_field%Source mapping not found for source_field : ' || temp.source_key_v) 
                        ELSE
                           1 
                     END
                     AS id_flag 
                  FROM
                     ...
                     LEFT JOIN
                        kpi.ms_org_master mpc 
                        ON temp.source_key_v = mpc.ref_code_v 
                  WHERE
                     temp.file_id_n = ? 
         )
         result 
   ...
   temp1 
   LEFT JOIN
      kpi.tr_daily_secondary_mobo_aggr daily1 
         ...
         (
            SELECT
               ...
               where
                  ...
                  temp.event_type_n,
                  temp.metrics_key_v,                  
				 21 as metrics_type_n, 111 AS metrics_id_n, temp.dimension_1_n, temp.dimension_2_n, temp.dimension_3_n, temp.dimension_4_n, temp.dimension_5_n, temp.dimension_6_n, temp.dimension_7_n, temp.dimension_8_n, temp.dimension_9_n, temp.dimension_10_n, temp.no_of_events_n, temp.source_key_v, COALESCE(( 
				 select
					reference_type_n 
				 from
					kpi.ms_actor_lookup_master 
				 where
					...
					GROUP BY
					   ...
					   LEFT JOIN
						  kpi.tr_monthly_secondary_mobo_aggr monthly1 
