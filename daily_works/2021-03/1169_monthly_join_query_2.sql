...
from 
	...
	from 
		(select 
			...
			temp.file_id_n, 
			case 
				when temp.metrics_key_v is not null and coalesce(met.metrics_id_n, 0) = 0 then kpi.update_analytics_failed_row('tr_temp_rguga_injection_aggr', temp.temp_id_n, 'metric_field%Metric mapping not found for metric_field : ' || temp.metrics_key_v)
				when temp.ref_1_key_v is not null and coalesce(ref1.metrics_id_n, 0) = 0 then kpi.update_analytics_failed_row('tr_temp_rguga_injection_aggr', temp.temp_id_n, 'metric_field%Metric mapping not found for metric_field : ' || temp.ref_1_key_v)				
	...
	left join kpi.tr_monthly_rguga_injection_aggr monthly1 on temp1.month_id_n = monthly1.month_id_n 
	...
	and temp1.ref_1_type_n = monthly1.ref_1_type_n 
	and temp1.ref_1_id_n = monthly1.ref_1_id_n;
