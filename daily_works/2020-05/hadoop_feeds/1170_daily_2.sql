SELECT
   ...
   CASE
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_1_n, 0) 
   END
   AS dimension_1_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_2_n, 0) 
   END
   AS dimension_2_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_3_n, 0) 
   END
   AS dimension_3_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_4_n, 0) 
   END
   AS dimension_4_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_5_n, 0) 
   END
   AS dimension_5_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_6_n, 0) 
   END
   AS dimension_6_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_7_n, 0) 
   END
   AS dimension_7_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_8_n, 0) 
   END
   AS dimension_8_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_9_n, 0) 
   END
   AS dimension_9_n, 
   CASE
      WHEN
      ...
      THEN
         NULL 
      ELSE
         COALESCE(temp1.dimension_10_n, 0) 
   END
   AS dimension_10_n, 
   CASE
      WHEN
...
FROM
            ...
            SELECT
               ...
               where
                  ...
                  where
                     ...
                     CASE
                        WHEN
                           ...
                           AND COALESCE(micro.lookup_id_n, 0) = 0 
                        THEN
                           kpi.update_analytics_failed_row('tr_temp_tertiary_sales_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v) 
                        WHEN
                           temp.source_key_v IS NOT NULL 
                           AND COALESCE(site.site_id_n, 0) = 0 
                        THEN
                           kpi.update_analytics_failed_row('tr_temp_tertiary_sales_aggr', temp.temp_id_n, 'source_field%Source mapping not found for source_field : ' || temp.source_key_v) 
                        ELSE
                           1 
                     END
                     AS id_flag 
                  FROM
                     ...
                     LEFT JOIN
                        kpi.ms_lookup_master micro 
                        ON micro.lookup_type_n = 
                        (
                           select
                              lookup_type_n 
                           from
                           ...
                           where
                              ext_lookup_type_n = 89
                        )
                        and temp.actor_key_v = micro.ext_ref_code_v 
                     LEFT JOIN
   ...
   temp1 
   LEFT JOIN
      kpi.tr_daily_tertiary_sales_aggr daily1 
