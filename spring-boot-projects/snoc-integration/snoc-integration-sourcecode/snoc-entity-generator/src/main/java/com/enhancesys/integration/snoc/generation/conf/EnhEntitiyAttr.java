...
public class EnhEntitiyAttr 
{
	...
	public EnhEntitiyAttr(String entityName, Node entityAttrNode) throws Exception 
	{
		...
		if(entityAttrNode.getAttributes().getNamedItem("mapped-by") !=null && entityAttrNode.getAttributes().getNamedItem("mapped-by").getNodeValue() != null)
			mapedBy = entityAttrNode.getAttributes().getNamedItem("mapped-by").getNodeValue().trim();

		if(entityAttrNode.getAttributes().getNamedItem("fetch") !=null && entityAttrNode.getAttributes().getNamedItem("fetch").getNodeValue() != null)
			fetchType = entityAttrNode.getAttributes().getNamedItem("fetch").getNodeValue().trim();
		if(entityAttrNode.getAttributes().getNamedItem("optional") !=null && entityAttrNode.getAttributes().getNamedItem("optional").getNodeValue() != null)
		...
		try 
		{
			...
			for (int root = 0 ; root < childNodes.getLength(); root++) 
			{
				...
				else if("java-attributes".equals(childNode.getNodeName()))
				{
					...
					{
						fieldFullName = dataTypescope + " " + "java.util.Set<"+EntityGeneration.getPACKAGE_VALUE()+ "." + dataType +"> " + name + " = new java.util.HashSet<"+EntityGeneration.getPACKAGE_VALUE()+ "." + dataType +">();";
						isOneTomany = true;						
					}
					else if("many-to-one".equals(type))
					{
						...
						if(EntityGeneration.CUSTOM_DATA_TYPES.contains(dataType))
						{
							...
							fieldFullName = dataTypescope + " " +EntityGeneration.getPACKAGE_VALUE() + "." +dataType + " "+ name + ";";	
						}
						else
				...
				else if ("generated-value".equals(childNode.getNodeName()))
				{
					...
					generatedValue = "@GeneratedValue(generator=\""+map.getNamedItem("generator").getNodeValue()+"\", strategy=GenerationType."+map.getNamedItem("strategy").getNodeValue()+")";
				}
				else if ("sequence-generator".equals(childNode.getNodeName()))
				{
					NamedNodeMap map = childNode.getAttributes();
					sequenceGenerator = "@SequenceGenerator(allocationSize="+map.getNamedItem("allocation-size").getNodeValue()+", initialValue="+map.getNamedItem("initial-value").getNodeValue()+", name=\""+map.getNamedItem("name").getNodeValue()+"\", sequenceName=\""+map.getNamedItem("sequence-name").getNodeValue()+"\")";
				}
			...
			if(javaAnnotations != null)
				for (int ann = 0 ; ann < javaAnnotations.getLength(); ann++) 
					if(javaAnnotations.item(ann).getNodeType()  != Node.COMMENT_NODE && !javaAnnotations.item(ann).getTextContent().trim().isEmpty())
						entityAttrAnnotations.add(javaAnnotations.item(ann).getTextContent().trim().replaceAll("<java-annotation>", "").replaceAll("</java-annotation>", "").replaceAll("\n", ""));
			...
			if(type.equals("one-to-many"))
				entityAttrAnnotations.add("@OneToMany(fetch=FetchType."+fetchType+", mappedBy=\""+mapedBy+"\")");
			if(type.equals("many-to-one"))
			{
				if(optionalType != null)
					entityAttrAnnotations.add("@ManyToOne(fetch=FetchType."+fetchType + ", optional = "+optionalType+")");
				else
			...
			if(node.getNodeName().equals("getter"))
			{
				...
				if(node.getChildNodes()!= null)
				{
					for(int x =0 ; x < node.getChildNodes().getLength() ; x ++)
					{
						...
						if(nodeX != null && nodeX.getNodeType() != Node.COMMENT_NODE && nodeX.getNodeName().equals("java-annotations"))
						{
							...
							if(annoList != null)
							{
								...
								{
									Node child = annoList.item(anno);
									//									System.out.println(nodeX.getNodeName() + " " + nodeX.getNodeValue() + " : " + child.getNodeName()  + " : " + child.getTextContent());
									if(child != null && (child.getNodeType() != Node.COMMENT_NODE) && child.getTextContent() != null && !child.getTextContent().isEmpty())
										if(getGetter().isEmpty())
											setGetter(getGetter() + "   " + child.getTextContent().trim().replaceAll("\t", "")+"\n");
										else
											setGetter(getGetter() + "   " + child.getTextContent().trim().replaceAll("\t", "")+"\n");
								}
							}
				...
				if(!EntityGeneration.CUSTOM_DATA_TYPES.contains(dataType) || isManyToOne)
					...
					setGetter(getGetter() + "   \n"
							+ "   public java.util.Set<"+type+"> get"+ startKeyWithUpperCase(name) +"()\n"
							+ "   {\n"
							+ "     this."+name+".size();\n"
...
//				System.out.println(entityName);
				...
				if(EntityGeneration.ENABLE_LISTENERS.get(dataType) == null && EntityGeneration.CUSTOM_DATA_TYPES.contains(entityName) && entityName.toLowerCase().contains(name.toLowerCase()))
				{
...
//					System.out.println(temp);
					if(temp != null)
						setSetter("\n"
								...
								+ "         old"+dataType+".remove"+temp.substring(0, temp.length()-1)+"(this);\n"
								+ "       \n"
								+ "       this."+name+" = arg;\n"
								+ "       \n"
								+ "       if(this."+name+" != null)\n"
								+ "         this."+name+".add"+temp.substring(0, temp.length()-1)+"(this);\n"
								+ "   }\n"
								+ "");
				}
				else
			...
			if(node.getNodeName().equals("enquirer"))
				setEnquirer( "   public Boolean has"+startKeyWithUpperCase(name).substring(0, startKeyWithUpperCase(name).length()-1)+"("+type+" arg)\n"
						+ "   {\n"
						+ "     return this."+name+".contains(arg);\n"
						+ "   }\n");

			if(node.getNodeName().equals("adder"))
				setAdder( "   public void add"+startKeyWithUpperCase(name).substring(0, startKeyWithUpperCase(name).length()-1)+"("+type+" arg)\n"
						+ "   {\n"
						+ "     if(arg == null || this."+name+".contains(arg))\n"
						+ "       return;\n"
						+ "     this."+name+".add(arg);\n"
						+ "     arg.set"+entityName+"(this);\n"
						+ "   }\n");

			if(node.getNodeName().equals("remover"))
				setRemover( "   public void remove"+startKeyWithUpperCase(name).substring(0, startKeyWithUpperCase(name).length()-1)+"("+type+" arg)\n"
						+ "   {\n"
						+ "     if(arg == null || !this."+name+".contains(arg))\n"
						+ "       return;\n"
		...
		if(typeId != null && !typeId.trim().isEmpty())
		{
			setIdAdapter( "   @XmlTransient\n"
					...
					+ "   {\n"
					+ "     @Override\n"
					+ "     public Long marshal("+entityName+" "+startKeyWithLowerCase(entityName)+") throws Exception \n"
					+ "     {\n"
					+ "       return "+startKeyWithLowerCase(entityName)+".get"+startKeyWithUpperCase(typeId)+"();\n"
					+ "     }\n"
					+ "     \n"
					+ "     @Override\n"
					+ "     public "+entityName+" unmarshal(Long arg) throws Exception \n"
					+ "     {\n"
					+ "       "+entityName+" "+startKeyWithLowerCase(entityName)+" = new "+entityName+"();\n"
		...
		if(isOneTomany || isManyToOne)
		{
			...
			if(isOneTomany)
				EntityGeneration.ENABLE_LISTENERS.put(dataType, "@EntityListeners(value={" + EntityGeneration.getPACKAGE_VALUE()+"."+entityName+"."+listener+".class" + "})");
//			if(isManyToOne)
			...
			String props = "   @XmlTransient\n"
					...
					+ "   }\n"
					+ "   \n"
					+ "   void _add"+nameWithUpper+"("+type+" arg)\n"
					+ "   {\n"
					+ "     this."+name+".add(arg);\n"
					+ "   }\n"
					+ "   \n"
	...
	public String findOutNextUpperCase(String key)
	{
		String temp = "";
