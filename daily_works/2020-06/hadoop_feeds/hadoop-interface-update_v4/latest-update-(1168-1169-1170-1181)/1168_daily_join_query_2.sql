--1168_DAILY_JOIN_QUERY_2=
select 
	1168 as interface_id_n,
	temp1.id_n,
	daily1.day_id_n,
	temp1.actor_type_n,
	temp1.actor_id_n,
...
from
	...
	from
		(select 
			...
			temp.file_id_n,
			case
				when temp.actor_key_v is not null and coalesce(site.site_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_daily_sso_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v)
				when temp.source_key_v is not null and coalesce(micro.lookup_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_daily_sso_aggr', temp.temp_id_n, 'source_field%Source mapping not found for source_field : ' || temp.source_key_v)
			else 1
			end as id_flag
		from 
	...
	left join kpi.tr_daily_daily_sso_aggr daily1 on temp1.id_n = daily1.day_id_n
