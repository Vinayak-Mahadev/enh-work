...
public class MongoDBDataSyncReader extends JobProcessor implements DataReader
{
	@Autowired
	...
	private static Logger log = Logger.getLogger(MongoDBDataSyncReader.class);

	public void init(String pipeLineName, String processorName, JSONObject processorConfig, JobConfiguration jobConfiguration, JSONObject jobParameters)
	{
		...
		try
		{
			...
			collectionArr = (JSONArray) _jobConfigData_.get(DataConstants.PRIMARY);
	...
	public boolean process(JSONObject jobObject) throws GenericProcessorException 
	{
		...
		try 
		{
			...
			catch(NoMoreDataException noMoreDataException)
			{
				...
				log.info(_pipeLineName_ + " : jobObject : " + jobObject);
				return true;
			}
