...
delete from kpi.mp_event_crossell_mapping where event_type_n between 302 and 302;
delete from kpi.mp_event_table_mapping where event_type_n between 302 and 302;
delete from kpi.mp_event_queue_mapping where event_type_n between 302 and 302;
delete from kpi.mp_event_frequency_mapping where event_type_n between 302 and 302;
delete from kpi.mp_event_dimension_mapping where event_type_n between 302 and 302;
delete from kpi.mp_event_metric_mapping where event_type_n between 302 and 302;
delete from kpi.mp_event_actor_mapping where event_type_n between 302 and 302;
delete from kpi.mp_event_format_mapping where event_type_n between 302 and 302;
delete from kpi.ms_event_type_master where event_type_n between 302 and 302;

-- MS_EVENT_TYPE_MASTER:
insert into kpi.ms_event_type_master (event_type_n, event_type_name_v, system_type_v, system_event_v) values (302, 'Mobii RGU-GA Injection', 'Hadoop', '1165');
...
drop table if exists kpi.tr_temp_mobii_rgu_ga_injection_aggr;
drop sequence if exists kpi.temp_mobii_rgu_ga_injection_aggr_seq;

create sequence kpi.temp_mobii_rgu_ga_injection_aggr_seq
  increment 1
  minvalue 1
...
create table kpi.tr_temp_mobii_rgu_ga_injection_aggr
(
  temp_id_n numeric(19,0) not null default nextval('kpi.temp_mobii_rgu_ga_injection_aggr_seq'::regclass),
  ...
  last_updated_time_dt timestamp with time zone not null default now(),
  constraint temp_mobii_rgu_ga_injection_aggr_seq_pkey primary key (temp_id_n)
);



drop index if exists kpi.tr_temp_mobii_rgu_ga_injection_aggr_file_idx;

create index tr_temp_mobii_rgu_ga_injection_aggr_file_idx
  on kpi.tr_temp_mobii_rgu_ga_injection_aggr
  using btree
  (file_id_n);


drop index if exists kpi.tr_temp_mobii_rgu_ga_injection_aggr_update_keys_idx;

create index tr_temp_mobii_rgu_ga_injection_aggr_update_keys_idx
  on kpi.tr_temp_mobii_rgu_ga_injection_aggr
  using btree
