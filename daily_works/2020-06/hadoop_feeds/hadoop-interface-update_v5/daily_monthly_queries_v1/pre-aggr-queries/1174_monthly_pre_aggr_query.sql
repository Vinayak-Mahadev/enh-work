...
update
	kpi.tr_monthly_total_rguga_aggr as monthly
set
...
from
	...
	from 
		kpi.tr_daily_total_rguga_aggr
	where 
		day_id_n in (select distinct(id_n) from kpi.tr_temp_total_rguga_aggr where file_id_n = ?)
	group by 
