...
public class TokuSeqUser /* implements DisposableBean */
{
	private static Logger log = Logger.getLogger(TokuSeqUser.class);
	...
	static
	{
		try
		{
			...
			//if(PropertiesFileLoader.getValueAsString("application.db.configuration").equalsIgnoreCase("toku")){
				TokuSeqUser.credential = MongoCredential.createMongoCRCredential(
			...
			/*else
			{
				TokuSeqUser.credential = MongoCredential.createScramSha1Credential(
