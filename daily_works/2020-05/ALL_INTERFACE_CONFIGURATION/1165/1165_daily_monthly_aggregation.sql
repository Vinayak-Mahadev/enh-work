...
FROM
            ...
            SELECT
               ...
               where
                  ...
                  where
                     ...
                     COALESCE(site.aggregation_id_n, 0) AS source_id_n,
                     temp.data_flag_n,
                     temp.instance_key_v,
                     ...
                     CASE
                        ...
                        THEN
                           kpi.update_analytics_failed_row('tr_temp_mobii_rgu_ga_injection_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v) 
                        WHEN
                           temp.source_key_v IS NOT NULL 
                           AND COALESCE(site.aggregation_id_n, 0) = 0 
                        THEN
                           kpi.update_analytics_failed_row('tr_temp_mobii_rgu_ga_injection_aggr', temp.temp_id_n, 'source_field%Source mapping not found for source_field : ' || temp.source_key_v) 
                        ELSE
                           1 
                     END
                     AS id_flag 
                  FROM
                     kpi.tr_temp_mobii_rgu_ga_injection_aggr temp 
                     LEFT JOIN
                        kpi.ms_org_master org 
                        ON temp.actor_key_v = org.ref_code_v 
                     LEFT JOIN
                        kpi.ms_lookup_master site 
                        ON site.lookup_type_n = (select lookup_type_n from kpi.ms_lookup_type_master where ext_lookup_type_n = 115) AND temp.source_key_v = site.ext_ref_code_v 
                  WHERE
                     temp.file_id_n = ? 
         )
         result 
   ...
   temp1 
   LEFT JOIN
      kpi.tr_daily_mobii_rgu_ga_injection_aggr daily1 
         ...
         (
            SELECT
               ...
               where
                  ...
                  temp.event_type_n,
                  temp.metrics_key_v,                  
				 21 as metrics_type_n, 112 AS metrics_id_n, temp.dimension_1_n, temp.dimension_2_n, temp.dimension_3_n, temp.dimension_4_n, temp.dimension_5_n, temp.dimension_6_n, temp.dimension_7_n, temp.dimension_8_n, temp.dimension_9_n, temp.dimension_10_n, temp.no_of_events_n, temp.source_key_v, COALESCE(( 
				 select
					reference_type_n 
				 from
					kpi.ms_actor_lookup_master 
				 where
					...
					WHERE
					   temp.file_id_n = ? 
					   and temp.status_flag_n = 0 ) result 
					where
					   result.id_flag = 1 
					GROUP BY
					   result.month_id_n, result.actor_type_n, result.actor_id_n, result.event_type_n, result.metrics_type_n, result.metrics_id_n, result.source_type_n, result.source_id_n, result.data_flag_n, result.instance_type_n, result.instance_id_n, result.file_id_n ) temp1 
					   LEFT JOIN
						  kpi.tr_monthly_mobii_rgu_ga_injection_aggr monthly1 
