-- To configure new actor and metric masters and lookups for new interface:(Gopinath)
-- KPI.MS_ACTOR_TYPE_MASTER:
delete from kpi.ms_actor_lookup_master where actor_type_n in (7,15);
delete from kpi.ms_actor_master where actor_type_n in (15);
...
delete from kpi.ms_metrics_lookup_master where metrics_type_n in (26,27);
delete from kpi.ms_metrics_type_master where metrics_type_n in (26,27);

insert into kpi.ms_metrics_type_master(metrics_type_n, metrics_type_name_v, system_type_v) values (26, 'Check-In Type', 'Custom');
insert into kpi.ms_metrics_type_master(metrics_type_n, metrics_type_name_v, system_type_v) values (27, 'Site Master', 'Custom');
...
insert into kpi.ms_metrics_lookup_master(metrics_type_n, reference_path_v, input_type_v, input_column_v, output_type_v, output_column_v, reference_type_n) values (27, 'select ? from kpi.ms_site_master where ?', 'string', 'ref_code_v', 'numeric', 'site_id_n', null);

-- To configure additional column in event metrics mapping table:(Gopinath)
-- KPI.MP_EVENT_METRIC_MAPPING:
alter table kpi.mp_event_metric_mapping drop column if exists data_flag_key_v, drop column if exists last_updated_by_n, drop column if exists last_updated_time_dt; 
alter table kpi.mp_event_metric_mapping add column data_flag_key_v character varying(255), add column last_updated_by_n numeric(19,0) not null default 1, add column last_updated_time_dt timestamp with time zone not null default now();

-- To update existing hadoop interface aggregation configuration:(Gopinath)
-- KPI.MP_EVENT_FORMAT_MAPPING:
delete from kpi.mp_event_format_mapping where event_type_n in (303,304,305,316);
insert into kpi.mp_event_format_mapping (event_type_n, format_type_v, format_v, definition_v, date_key_v, date_format_v, source_key_v, source_type_n, instance_key_v, instance_type_n) values(303, 'Csv', 'DATE|MICRO|SITE_ID|ID_OUTLET|QTY|AMOUNT|QSSO_STATUS', 'Aggregation', 'DATE', 'yyyyMMdd', 'MICRO', 13, 'SITE_ID', 27);
...
insert into kpi.mp_event_format_mapping (event_type_n, format_type_v, format_v, definition_v, date_key_v, date_format_v, source_key_v, source_type_n, instance_key_v, instance_type_n) values(305, 'Csv', 'DATE|MICRO|SITE_ID|OUTLET|AMOUNT', 'Aggregation', 'DATE', 'yyyyMMdd', 'MICRO', 13, 'SITE_ID', 27);
insert into kpi.mp_event_format_mapping (event_type_n, format_type_v, format_v, definition_v, date_key_v, date_format_v, source_key_v, source_type_n, instance_key_v, instance_type_n) values(316, 'Csv', 'DATE|MICRO|SITE_ID|OUTLET|HIT|AMOUNT|QURO_STATUS', 'Aggregation', 'DATE', 'yyyyMMdd', 'MICRO', 13, 'SITE_ID', 27);
...
-- KPI.MP_EVENT_METRIC_MAPPING:
delete from kpi.mp_event_metric_mapping where event_type_n in (303,316);
insert into kpi.mp_event_metric_mapping (event_type_n, configuration_n, metric_key_v, description_v, metrics_type_n, metrics_id_n, selection_v, condition_key_v, condition_val_v, data_flag_key_v, data_flag_n, rollup_flag_bl, delimiter_v) values (303, 1, 'ID_OUTLET', 'Overall', 21, array[110], 'Fixed', null, null, 'QSSO_STATUS', 0, true, null);
insert into kpi.mp_event_metric_mapping (event_type_n, configuration_n, metric_key_v, description_v, metrics_type_n, metrics_id_n, selection_v, condition_key_v, condition_val_v, data_flag_key_v, data_flag_n, rollup_flag_bl, delimiter_v) values (316, 1, 'OUTLET', 'Overall', 21, array[110], 'Fixed', null, null, 'QURO_STATUS', 0, true, null);
...
insert into kpi.ms_event_type_master (event_type_n, event_type_name_v, system_type_v, system_event_v) values (319, 'DSSF Attendance', 'Hadoop', '1185');
