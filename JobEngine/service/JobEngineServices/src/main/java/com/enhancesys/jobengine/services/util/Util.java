package com.enhancesys.jobengine.services.util;
...
public class Util 
{
	...
	{
		log.info("getStartOfTheDay:: Parsing Object : "+object);
		Date date = null;
		...
		log.info("getStartOfTheDay:: Returning Object : "+date);
		return date;
	}

	public static Date getEndOfTheDay(Object  object) throws Exception 
	{
		log.info("getEndOfTheDay:: Parsing Object : "+object);
		Date date = null;
		...
		log.info("getEndOfTheDay:: Returning Object : "+date);
		return date;
	}
...
//						log.error("inside format..");
						...
						if(fromFormat != null)
						{
							...
							{
								log.error("To Date Format should not be empty..");
							...
							if(paramObject.get(DataConstants.FROM_DATE_FORMAT) != null && !paramObject.get(DataConstants.FROM_DATE_FORMAT).toString().trim().isEmpty())
							{
								...
								if(fromFormat != null)
								{
									...
									if(paramObject.get(DataConstants.PARAM_VALUE) instanceof JSONArray)
									{
										...
										{
											log.error("Date params should be 2 always.." + paramValues);
...
//											log.error("paramValues.get(0).toString() : " + paramValues.get(0).toString());
											...
											{
												values = Util.getFieldValue(paramValues.get(0).toString(), dataObject);
												if(values != null && !values.isEmpty())
													fieldValue = values.iterator().next().toString();   
											}
											else 
...
//											log.error("fieldValue : " + fieldValue);
											...
											{
												values = Util.getFieldValue(paramValues.get(1).toString(), dataObject);
												if(values != null && !values.isEmpty())
													fieldValue = values.iterator().next().toString();   
											}
											else 
										...
										dateQuery = new BasicDBObject("$gte", Util.getStartOfTheDay(startDate)).append("$lte", Util.getEndOfTheDay(endDate));
...
//										log.error("paramObject : " + paramObject);
												...
												&& DataConstants.REPLACE.equalsIgnoreCase(paramObject.get(DataConstants.PARAM_VALUE_TYPE).toString().trim()))
										{
											...
											{
												values = Util.getFieldValue(paramObject.get(DataConstants.PARAM_VALUE).toString(), dataObject);
												if(values != null && !values.isEmpty())
													fieldValue = values.iterator().next().toString();   
											}
											else 
											...
											if(paramObject.get(DataConstants.OPERATION_TYPE) != null && !paramObject.get(DataConstants.OPERATION_TYPE).toString().trim().isEmpty())
											{
												...
												if(DataConstants.START_OF_THE_DAY.equalsIgnoreCase(timeFormatType))
													queryReqDate = Util.getStartOfTheDay(startDate);
												else if(DataConstants.END_OF_THE_DAY.equalsIgnoreCase(timeFormatType))
													queryReqDate = Util.getEndOfTheDay(startDate);
...
//												log.error("timeFormatType : " + timeFormatType + " : queryReqDate : " + queryReqDate);
							...
							else
							{
								if(paramObject.get(DataConstants.PARAM_VALUE) instanceof JSONArray)
								{
									...
									dateQuery = new BasicDBObject("$gte", Util.getStartOfTheDay(calendar.getTime())).append("$lte", Util.getEndOfTheDay(calendar2.getTime()));
									queryObject.put(paramObject.get(DataConstants.PARAM_NAME).toString(), dateQuery);
								}
								else
								{
									...
									dateQuery = new BasicDBObject("$gte", Util.getStartOfTheDay(calendar.getTime())).append("$lte", Util.getEndOfTheDay(calendar.getTime()));
									queryObject.put(paramObject.get(DataConstants.PARAM_NAME).toString(), dateQuery);
								}
							}
						}
...
//				columnName = CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.LOWER_CAMEL, metaData.getColumnName(i));
	...
	public static void mergeObject(DBObject mainObject, JSONObject subObject)
	{
		for(Object key : subObject.keySet())
		{
			if(mainObject.containsKey(key.toString()))
			{
