-- 1171_DAILY_JOIN_QUERY_1=
select 
	1171 as interface_id_n,
	temp1.id_n,
	daily1.day_id_n,
	temp1.actor_type_n,
	temp1.actor_id_n,
   ...
   from
		(select 
			...
			case 
				...
				then kpi.update_analytics_failed_row('tr_temp_closing_balance_aggr', temp.temp_id_n, 'actor_field%Actor mapping not found for actor_field : ' || temp.actor_key_v)
			else 1
			end as id_flag
		from 
			kpi.tr_temp_closing_balance_aggr as temp
	...
	left join kpi.tr_daily_closing_balance_aggr daily1 on temp1.id_n = daily1.day_id_n
