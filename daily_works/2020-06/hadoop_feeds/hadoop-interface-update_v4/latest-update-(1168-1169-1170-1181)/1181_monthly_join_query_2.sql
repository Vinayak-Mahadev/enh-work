	...
	from
		(select 
			...
			case
				...
				then kpi.update_analytics_failed_row('tr_temp_uro_20k_aggr', temp.temp_id_n, 'actor_field%Source mapping not found for actor_field : ' || temp.actor_key_v)
				when temp.actor_key_v is not null and coalesce(micro.lookup_id_n, 0) = 0 
				then kpi.update_analytics_failed_row('tr_temp_uro_20k_aggr', temp.temp_id_n, 'source_field%Actor mapping not found for source_field : ' || temp.source_key_v)
			else 1
			end as id_flag
		from 
